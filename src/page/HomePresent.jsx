...
const Home = () => {
    ...
    useEffect(() => {
      client.get(`/list/projects`)
      .then (function (response) {
          console.log(response.data)
          setprojects(prevState => response.data)
    ...
    console.log('ini adalah', projects)

    return (
        <>
         <section
    className="h-full w-full border-box  transition-all duration-500 linear bg-white"
    >
      ...
      <div>
        <div className="lg:px-32 md:px-8  sm:px-12 px-5 pt-20 pb-12 mx-auto  mb-6">
          ...
          { projects.data ? projects.data.map((item, index) => {
            return  (
              ...
              <div class="rounded-lg overflow-hidden bg-white shadow-xl">
                <div className="relative overflow-hidden pb-60">
                  <img
                    ...
                    src={item.image ? item.image : 'https://ik.imagekit.io/uqhk3wr7kix/image_9_W5okuV2nW.png?updatedAt=1627561823080'}
                    alt=""
                  />
                </div>
                <div className="relative ">
                  <div className="py-6 px-4">
                    ...
                    <p className="my-2 text-xs font-light  text-gray-600 ">{item.short_description}</p>
                  
                    <div className="mt-2 mb-2">
                      <div className="">
                          <span className="text-xl font-bold text-blue-400 ">Rp. {new Number(item.current_amount).toLocaleString('id-ID')}</span>
                        <span className="px-2 text-grey-100 text-xs font-light">Terkumpul Dari Rp. {new Number(item.goal_amount).toLocaleString('id-ID')}</span>
                      </div>
                    ...
                    <div className="flex lg:flex-row flex-col -m-4 mt-2 mb-2">
                      ...
                      <h3 class="text-2xl font-bold text-gray-800">{item.donations ? item.donations.length : 0}  <span className="text-sm font-light text-black-200 ">Donasi</span></h3>
                      </div>
